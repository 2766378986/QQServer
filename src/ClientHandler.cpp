...
ClientHandler::ClientHandler(QTcpSocket * socket, Server * srv) : socket(socket), srv(srv) {
    databaseConnect();
}
...
void ClientHandler::addClient(const QString & account, ClientHandler * client) {
...
void ClientHandler::onReadyRead()//读取用户发送的数据
{
    ...
    while (jsonData.endsWith("END")) {
        ...
        if (jsonObj["tag"] == "login") {
            ...
            dealAddFriend(jsonObj);//处理用户发送添加好友申请的功能
        }
...
QString ClientHandler::pximapToBase64(const QPixmap &pixmap)//图片转换成base64格式
{
...
void ClientHandler::dealAvator(const QString &avator)//上传头像
{
    ...
    if(!qry.exec()){//如果执行失败
        ...
        qjsonObj["answer"]="registerFailed";
        jsonDoc = QJsonDocument(qjsonObj);
        jsonData = jsonDoc.toJson();
        //添加标识符
        QByteArray messageWithSeparator = jsonData + "END";
        ...
        QJsonObject qjsonObj;
        qjsonObj["tag"] = "register";
        qjsonObj["answer"] = "registerSuccess";
...
void ClientHandler::dealSearchAccount(const QJsonObject &json)//处理用户搜索账户信息的功能
{
    QMutexLocker locker(&dbMutex);//锁住互斥量以确保线程安全
    QSqlQuery qry(db);
    QJsonObject qjsonObj;
...
void ClientHandler::dealAddFriend(const QJsonObject &json)//处理用户发送添加好友申请的功能
{
...
void ClientHandler::forwardAddFriendRequest(const QJsonObject &json)//向在线用户转发用户好友申请
{
...
void ClientHandler::forwardRequestPass(const QJsonObject &json)//向在线用户更新他的好友列表(他的好友申请被通过)
{
    //json[account]是通过人
    QMutexLocker locker(&dbMutex);//锁住互斥量以确保线程安全
    QSqlQuery qry(db);
...
void ClientHandler::forwardBeDeleted(const QJsonObject &json)//向在线用户更新他的好友列表(他被删除了)
{
    //json[account]是谁删除了你
    QMutexLocker locker(&dbMutex);//锁住互斥量以确保线程安全
    QJsonObject qjsonObj;
...
void ClientHandler::forwardKickedOffline(const QJsonObject &json)//把在线用户挤下线
{
    QMutexLocker locker(&dbMutex);//锁住互斥量以确保线程安全
    QJsonObject qjsonObj;
...
void ClientHandler::forwardMessages(const QJsonObject &json)//向在线用户发送聊天消息
{
    QMutexLocker locker(&dbMutex);//锁住互斥量以确保线程安全
    //发送消息
